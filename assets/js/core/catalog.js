...
export function getArtistProfile(slug){
  ...
  if(!tracks.length){
    return { slug, name: slug, description: 'No profile yet.', gradient: 'linear-gradient(135deg,#17121a,#43253c)' };
  }
  return {
    slug,
...
export function getArtistSlugs(){
  ...
  return Array.from(set).sort((a,b) => getArtistName(a).localeCompare(getArtistName(b)));
}
...
export function getStationTracks(index){
  ...
  const seeded = indexes.map(i => catalogTracks[i]).filter(Boolean);
  if(seeded.length){ return seeded; }
  return catalogTracks.slice(index % Math.max(1, catalogTracks.length), (index % Math.max(1, catalogTracks.length)) + 6);
}
...
export function searchCatalog(query){
  ...
  return { tracks, artists, stations: stationMatches };
}
